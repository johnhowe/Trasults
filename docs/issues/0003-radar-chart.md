...
p75 and p50 are suppressed when fewer than 10 completed routines exist in the form window.
...
A shallow `radar_data(db_path, given_name, surname, discipline, form_months)` helper returns `{axes, bounds, field_median, athlete: {pb, top5_mean, p75, p50}, n_completed}` with all axis values already oriented so "larger = better."
...
- [ ] When n_completed < 10 in form window, p75 and p50 rings are suppressed; PB and Top-5 still render if their inputs exist.
