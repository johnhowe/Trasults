...
A new "Depth" section is added to the top of the `/dashboard` athlete page. The section centres on a **Radar** chart that collapses an athlete's performance shape into one visual, surrounded by a **Rolling peak** (best-3-of-last-10 routine totals), a **Crash rate** indicator, paired **rolling-peak-and-crash trend lines** showing how those two move over the athlete's career, **Trade-off scatters** revealing the D/E/ToF relationships routine-by-routine, and two **skill heatmaps** showing exactly which skill positions are losing points and how that pattern varies across routine class.
...
A new **Lookback window** control (past N months) sits alongside the existing **Cohort filter** (year_from/year_to) as a second, independent time concept. The cohort filter bounds the comparison field for cross-athlete panels; the lookback window bounds the athlete-scoped recency lens for the new Depth view. (The window was originally named "form window"; renamed to avoid colliding with the execution-form meaning of E-score — see ADR-0001.)
...
5. As a coach, I want the Radar's percentile rings (p75, p50) to be suppressed when there are fewer than 10 routines in the lookback window, so that I don't read a percentile based on three data points.
...
7. As a coach, I want a **Rolling peak** showing the mean of my athlete's best 3 routine totals from their last 10 completed routines, so that I have a single number summarising current performance level that's robust to one bad routine.
8. As a coach, I want the same rolling peak plotted as a **trend line** across the athlete's full career, so that I can see whether they're at a peak, in a slump, or on a plateau.
9. As a coach, I want a **Crash rate** indicator showing the share of crashes in the lookback window, so that I can see the reliability side of current performance at the same glance as the rolling peak.
10. As a coach, I want a **rolling crashes-per-10-routines line** paired visually with the rolling-peak trend line, so that I can scan both at once — peak going up is good, crashes going up is bad.
11. As a coach with crashes-as-routine, I want a string of crashes to show up as a *stalled* rolling-peak trend (because they're excluded from the best-of-3 pool) rather than a *collapsed* one, so that the rolling peak reflects underlying capability and the crash line carries the reliability signal separately.
...
20. As a coach, I want a **Lookback window** control (past N months, default 12) that updates the Radar, KPIs, scatters, and timeline heatmap, so that I can ask "how is my athlete looking right now?" independently of the cohort filter.
...
22. As a coach, I want the Lookback window and the Cohort filter to be visually distinct in the UI and labelled differently, so that I never confuse the comparison-field question with the recency-lens question.
...
27. As a coach, I want the Rolling peak's "best 3" pool to exclude crashes, so that recent reliability problems don't depress my reading of current technical capability.
...
30. As an analyst looking at the data, I want the same crash predicate to apply consistently across every new visual (Radar, scatters, heatmap timeline, rolling-peak trend), so that "completed routines only" is a single load-bearing concept and not redefined per panel.
...
- **`rolling_peak`** — pure-functional module computing both the rolling-peak series and the crash rate series from a chronological routine list:
...
- **`lookback_window`** — module enforcing the ADR-0001 boundary between cohort filter and lookback window:
...
- `radar_data` — returns `{axes, athlete: {pb, top5_mean, p75, p50}, field_median, bounds, n_completed}` for the current lookback window.
- `rolling_peak_and_crash_series` — returns the rolling best-3-of-10 series and the crashes-per-10 series across the athlete's full career (career view; not window-scoped). `lookback_kpi_data` returns the corresponding single "current" values for the KPI tiles scoped to the lookback window.
...
- New query parameter: `lookback_months` (default 12). Distinct from `year_from`/`year_to` per ADR-0001.
- Existing cohort filter continues to drive the existing eight panels.
- Lookback window drives the new Depth panels.

### Template changes

- `dashboard.html`: new "Depth" section is inserted between the athlete-header card and the existing `.panel-grid`. Inside it:
  - A KPI row (Rolling peak number, Crash rate number, lookback window selector).
  - The Radar panel (wide).
  - The paired rolling-peak / crash trend lines (wide, two-line chart).
...
- New tokens for: lookback-window control styling, KPI tile, heatmap colour ramp (red/heat scale), compulsory-strip two-tone (compulsory=pale, voluntary=dark).
...
- The rolling-peak / crash trend pair is one Chart.js line chart with two y-axes (rolling peak on the left, crash rate on the right) to keep them visually paired.
...
- The lookback window and the cohort filter never share a code path. ADR-0001 is enforced by the module boundary: `lookback_window` filters routines; `cohort_filter` (existing) emits SQL. They have different return types on purpose.
...
- The shallow per-panel query helpers (`radar_data`, `rolling_peak_and_crash_series`, etc.) are tested via the existing `test_dashboard.py` smoke-test pattern — invoke against the live DB, assert structure and reasonable ranges, not exact values. Most of their logic lives in the deep modules that already have unit tests.
- The Flask route gets one extension to `test_dashboard.py`: hit `/dashboard?given_name=X&surname=Y&lookback_months=12`, assert HTTP 200 and that the response contains the expected new panel IDs (`#c-radar`, `#c-trend`, `#c-scatter-*`, `#c-heatmap-timeline`, `#c-heatmap-class`).
...
- **Configurable rolling-peak window** — the 3-of-10 ratio and the lookback-window default of 12 months are fixed for v1. Adjustability is deferred until there's evidence the defaults are wrong.
- **Per-stage / per-routine-class crash rate breakdown** — the dashboard surfaces overall crash rate in the lookback window plus the rolling line. A breakdown by qual/final/compulsory was considered (Q7) and explicitly deferred.
...
- The two ADRs `docs/adr/0001-lookback-window-separate-from-cohort-filter.md` and `docs/adr/0002-radar-uses-sport-empirical-absolute-scale.md` should be read before implementation. They explain the two architectural decisions a future maintainer is most likely to "fix" by mistake.
- `CONTEXT.md` carries the canonical glossary used throughout this PRD: Crash, Radar, Rolling peak, Crash rate, Cohort filter, Lookback window, Compulsory vs voluntary, Trade-off scatters, Skill heatmaps. The PRD's terminology must stay in lockstep with that glossary.
