"""Unit tests for form_window."""

from datetime import date

import form_window as fw


def test_parse_months_valid_integer():
    assert fw.parse_months('6') == 6
    assert fw.parse_months('24', default=12) == 24


def test_parse_months_missing_or_blank_returns_default():
    assert fw.parse_months(None) == 12
    assert fw.parse_months('') == 12
    assert fw.parse_months(None, default=6) == 6


def test_parse_months_malformed_returns_default():
    assert fw.parse_months('abc') == 12
    assert fw.parse_months('1.5') == 12
    assert fw.parse_months('12 months') == 12


def test_parse_months_nonpositive_returns_default():
    assert fw.parse_months('0') == 12
    assert fw.parse_months('-3') == 12
...
def test_filter_routines_inside_and_outside_window():
    ...
    out = fw.filter_routines(routines, months=6, now=date(2024, 6, 15))
